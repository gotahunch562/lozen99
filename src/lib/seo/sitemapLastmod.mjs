...
// Reads date from script block. Checks in order:
// 1. dateModified: "YYYY-MM-DD"  (jsonLd calls)
// 2. publishedDateIso = "YYYY-MM-DD"  (press release pages)
function getDateFromJsonLdCall(fileContent) {
  const dateModified = fileContent.match(/dateModified:\s*["'](\d{4}-\d{2}-\d{2})["']/);
  if (dateModified) return dateModified[1];
  const publishedDateIso = fileContent.match(/publishedDateIso\s*=\s*["'](\d{4}-\d{2}-\d{2})["']/);
  if (publishedDateIso) return publishedDateIso[1];
  return "";
}
...
// Reads src/content/services/*.md and maps /services/<slug>/ -> lastmod.
// These pages are generated by [...slug].astro so sitemapLastmod never sees
// them via the pages walk — they must be read from the content directory.
function getServicesLastmodMap() {
  const lastmodMap = new Map();
  if (!existsSync(SERVICES_DIR)) return lastmodMap;
  const files = readdirSync(SERVICES_DIR).filter((f) => [".md", ".mdx"].includes(extname(f)));
  for (const file of files) {
...
const servicesLastmodMap = getServicesLastmodMap();
...
export function serializeSitemapItem(item) {
  ...
  const lastmod =
    ...
    servicesLastmodMap.get(normalizedUrl) ||
